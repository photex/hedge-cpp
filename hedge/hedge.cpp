...
public:
  ...
  void remove(TElementIndex index)
  {
    ...
    if (element_at_index != nullptr)
    {
      ...
      auto constexpr max_generation = std::numeric_limits<generation_t>::max();
...
face_index_t mesh_builder_t::add_triangle(point_index_t pindex0,
                                          point_index_t pindex1,
                                          point_index_t pindex2)
{
  auto root_eindex =
    start_edge_loop(pindex0).add_point(pindex1).add_point(pindex2).close();
...
normal_t vertex_fn_t::normal() const
{
  ...
  {
    LOG(WARNING) << "Retreiving normal value from invalid element proxy";
    return normal_t::Zero();
  }
}

normal_t vertex_fn_t::calculate_normal()
{
  auto* elem = element();
  if (!elem)
  {
    LOG(ERROR) << "Unable to calculate normal for invalid vertex.";
    return normal_t::Zero();
  }
  auto p0 = point().position();
  auto p1 = edge().next().vertex().point().position();
  auto p2 = edge().prev().vertex().point().position();

  auto A = p1 - p0;
  auto B = p2 - p0;

  elem->normal = A.cross(B).normalized();
  return elem->normal;
}
...
normal_t face_fn_t::calculate_normal()
{
  ...
  if (elem)
  {
    ...
    {
      edge_fn_t edge(_kernel, eindex);
      if (edge)
      {
        ++edge_count;
        accum += edge.vertex().calculate_normal();
      }
    }
    elem->normal = (accum / edge_count).normalized();
    return elem->normal;
  }
  else
  {
